...
| --- | --- | --- | --- |
| Desktop | ExtJS `synoips.js` + `ui/config` `type: app` | none | pack official UI + `tps-bridge.js` |
...
## 5. What this PoC implements (8.0.6-0011)

Compatibility layer on **vanilla Suricata 8.0.6** (AF_PACKET IDS):

1. **Pack official UI** from `unpacked/package/ui/` (`synoips.js`, texts, help, icons, `index.conf`).
2. **`tps-bridge.js`** — original hook: `sendWebAPI`, `downloadWebAPI`, `pollReg`/`pollUnreg`, `SYNO.API.Request`, `Ext.Ajax`, and a best-effort `SYNO.API.Info` inject. Forwards `SYNO.TPS.*` to `http://NAS:19557/`.
3. **`compat.py` + `tpsweb`** — official envelopes: Event task_id, Sensor state names, Signature `signatures` / Policy `list` / int types, Statistic buckets, Device `device_list`, lowercase vars, backup download.
4. **`ingest.py`** — tail `eve.json` into SQLite (Barnyard2-ish).
5. **`compiler.py`** — `signature.conf` + `policy_*` → `var/rules/suricata.rules` + reload.
6. **`SYNO.TPS.lib`** copied for Info listing. **No** aarch64 `.so`.

`INFO.dsmappname=SYNO.SDS.TPS.Application` so the Start Menu launches the official window after a DSM logout/login.
