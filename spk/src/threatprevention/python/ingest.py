...
def insert_alert(conn, obj):
    ...
    cid = cur.lastrowid
    conn.execute(
        """INSERT OR REPLACE INTO iphdr(sid, cid, ip_src, ip_dst, ip_ver, ip_proto) VALUES (1,?,?,?,?,?)""",
        (cid, ip_to_int(src), ip_to_int(dst), 6 if ":" in src else 4, proto),
    )
    if proto == 6:
        conn.execute(
            "INSERT OR REPLACE INTO tcphdr(sid, cid, tcp_sport, tcp_dport, tcp_flags) VALUES (1,?,?,?,0)",
            (cid, int(obj.get("src_port") or 0), int(obj.get("dest_port") or 0)),
        )
    elif proto == 17:
        conn.execute(
            "INSERT OR REPLACE INTO udphdr(sid, cid, udp_sport, udp_dport) VALUES (1,?,?,?)",
            (cid, int(obj.get("src_port") or 0), int(obj.get("dest_port") or 0)),
        )
    elif proto in (1, 58):
        icmp = obj.get("icmp") or {}
        conn.execute(
            "INSERT OR REPLACE INTO icmphdr(sid, cid, icmp_type, icmp_code) VALUES (1,?,?,?)",
            (cid, int(icmp.get("type") or 0), int(icmp.get("code") or 0)),
        )
    payload = obj.get("payload_printable") or obj.get("payload") or ""
    if payload:
        conn.execute("INSERT OR REPLACE INTO data(sid, cid, data_payload) VALUES (1,?,?)", (cid, payload[:4096]))
    mac = ether.get("src_mac") or ""
    if mac:
        name = src or mac
